...
function formatResearchError(error: unknown): string {
	if (error instanceof BreadboardRunConfigError) {
		return `${BREADBOARD_PRODUCT_IDENTITY.displayName} configuration error [${error.code}/${error.field}]: ${error.message}`;
	}
		...
		try {
			...
			if (connected.kind !== "ready") {
				...
				try {
					...
					operationOutput = `${JSON.stringify(result)}\n`;
					operationExitCode = result.exit_code;
				} catch (error) {
			...
			if (!operationFailed && cleanupError === undefined && operationOutput !== undefined) {
