...
export class RosterView {
	...
	#summaryLines(width: number): string[] {
		...
		if (metrics.reportedAgents === 0) {
			lines.push(
				...wrapTextWithAnsi(
					theme.fg("dim", `Usage —${theme.sep.dot}0/${agents.length} measured`),
					Math.max(1, width),
				),
			);
			return lines;
	...
	#renderDetailPanel(ref: AgentRef | undefined, width: number, rows: number): string[] {
		...
		section("Lineage");
		add(
			`Spawned by ${sanitizeDisplayText(ref.parentId ?? "Main")}${children.length > 0 ? ` · ${children.length} children` : ""}`,
		);
	...
	#renderEntry(
		...
		if (viewMode === "roster" && ref.parentId && ref.parentId !== "Main")
