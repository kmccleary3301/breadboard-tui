...
import { lifecycleFailure, lifecycleState, type LifecycleResult } from "../../src/breadboard/lifecycle/lifecycle-state";
...
function wireEvent(sequence: number, type: string, payload: unknown, turnId: string | null = "turn-1"): LoggedSessionEvent {
...
function openedSession(events: readonly LoggedSessionEvent[], submissions: unknown[] = []): OpenedSession {
	...
	return {
		...
		async submit(input) {
			...
			return { ...receipt, clientMessageId: (input as { clientMessageId?: string }).clientMessageId as ClientMessageId };
		},
		async cancel(): Promise<CancellationReceipt> {
			return { disposition: "cancellation_requested", originalDisposition: "cancellation_requested", requestId: "cancel-1" } as never;
		},
...
describe("BBOMP-CORE-52 — SDK event envelope, decoding, ordering, and cursor behavior (12)", () => {
	test("[fast] decodes the canonical turn-start envelope into typed fields", () => {
		expect(turnStarted).toMatchObject({ kind: "turn_started", sequence: 1, sessionId: "session-1", inputId: "input-1", turnId: "turn-1" });
	});
	...
	test("[fast] rejects an envelope with a missing stable cursor", () => {
		expect(() => decodeLoggedSessionEvent({ id: "event-1", seq: 1, session_id: "session-1", type: "turn_start", payload: {} })).toThrow();
	});
	test("[fast] rejects an unknown event family instead of guessing its meaning", () => {
		expect(() => decodeLoggedSessionEvent({ stable_cursor: true, id: "event-2", seq: 2, session_id: "session-1", type: "future.event", payload: {} })).toThrow();
	});
	test("[fast] serializes the same event deterministically", () => {
		expect(Buffer.from(serializeLoggedSessionEvent(turnStarted))).toEqual(Buffer.from(serializeLoggedSessionEvent(turnStarted)));
	});
	test("[fast] event serialization keeps the event envelope fields", () => {
		const decoded = JSON.parse(Buffer.from(serializeLoggedSessionEvent(turnStarted)).toString("utf8")) as Record<string, unknown>;
	...
	test("[fast] changes the event digest when the durable sequence changes", async () => {
		expect(await digestLoggedSessionEvent(turnStarted)).not.toBe(await digestLoggedSessionEvent(wireEvent(9, "turn_start", {})));
	});
	test("[fast] orders events by their durable sequence rather than arrival order", () => {
		const events = [wireEvent(3, "turn_completed", {}), turnStarted, wireEvent(2, "assistant.message.delta", { text: "x" })];
...
describe("BBOMP-CORE-52 — session create/resume/reconnect/cancel/replay (12)", () => {
	test("[fast] creates a session through the canonical port without changing the request", async () => {
		...
		const port = new CanonicalE4SessionPort({ create: async value => { received = value; return runtime; }, attach: async () => runtime }, { onLateCloseError: () => {} });
	...
	test("[fast] resumes an existing session through attach with its opaque ID", async () => {
		let received: unknown;
		const runtime = openedSession([]);
		const port = new CanonicalE4SessionPort({ create: async () => runtime, attach: async value => { received = value; return runtime; } }, { onLateCloseError: () => {} });
	...
	test("[fast] aborts a session open before issuing a backend request", async () => {
		let calls = 0;
		const runtime = openedSession([]);
		const port = new CanonicalE4SessionPort({ create: async () => { calls += 1; return runtime; }, attach: async () => runtime }, { onLateCloseError: () => {} });
		const abort = new AbortController();
		abort.abort();
		await expect(port.open({ kind: "create", request: { configPath: "agent.yaml" } }, abort.signal)).rejects.toThrow();
		expect(calls).toBe(0);
	});
	test("[fast] closes a session exactly once when an open is aborted late", async () => {
		...
		const port = new CanonicalE4SessionPort({ create: async () => pending, attach: async () => pending }, { onLateCloseError: () => {} });
		const abort = new AbortController();
		const opening = port.open({ kind: "create", request: { configPath: "agent.yaml" } }, abort.signal);
		abort.abort();
		resolve({ ...openedSession([]), close: async () => { closeCalls += 1; } });
	...
	test("[fast] preserves a caller cursor when reconnecting event observation", async () => {
		...
		const wrapped: OpenedSession = { ...session, async *events(request) { for await (const event of originalEvents(request)) observed.push(event.sequence); } };
	...
	test("cancellation requests retain an explicit user-requested reason", async () => {
		const calls: unknown[] = [];
		const session = { ...openedSession([]), async cancel(request: unknown) { calls.push(request); return {} as CancellationReceipt; } } as OpenedSession;
	...
	test("closing an opened session is idempotent at the port boundary", async () => {
		...
		const port = new CanonicalE4SessionPort({ create: async () => runtime, attach: async () => runtime }, { onLateCloseError: () => {} });
	...
	test("a session snapshot keeps terminal turn outcome and replay cursor together", () => {
		const snapshot = { sessionId: "session-1", status: "completed", turnAdmission: "idle", activeTurnId: null, queuedTurnCount: 0, terminalTurns: [{ inputId: "input-1", turnId: "turn-1", outcome: "completed", originalDisposition: "started" }], ...validReplayFacts() };
...
describe("BBOMP-CORE-52 — tool result, terminal outcome, and exact-once visibility (8)", () => {
	test("[fast] decodes a tool call with its stable call ID", () => {
		const event = wireEvent(4, "tool_call", { call_id: "call-1", tool: "read", arguments: { path: "file" }, action: null, diff_preview: null, progress: null });
	...
	test("[fast] decodes a tool result with explicit error status", () => {
		const event = wireEvent(5, "tool.result", { call_id: "call-1", tool: "read", status: "failed", error: true, result: null, artifact_ref: null });
		expect(event).toMatchObject({ kind: "tool_result_observed", payload: { callId: "call-1", error: true, status: "failed" } });
	});
...
describe("BBOMP-CORE-52 — process exit, signal, cleanup, and host-terminal restoration (8)", () => {
	test("[fast] child process environment is an exact minimal allowlist", () => {
		expect(lifecycleChildEnvironment("launch-1")).toEqual({ PATH: "/usr/bin:/bin", BREADBOARD_ENGINE_LAUNCH_ID: "launch-1", BREADBOARD_LIFECYCLE_BOOTSTRAP_FD: "3" });
	});
...
describe("BBOMP-CORE-52 — credential and redaction invariants (4)", () => {
	test("config digests do not change when only a process credential rotates", () => {
		const first = resolveBreadboardRunConfig({ ...configInput, cli: { engineMode: "remote", engineUrl: "https://engine.example" }, environment: { BREADBOARD_API_TOKEN: "s".repeat(32) } });
		const second = resolveBreadboardRunConfig({ ...configInput, cli: { engineMode: "remote", engineUrl: "https://engine.example" }, environment: { BREADBOARD_API_TOKEN: "t".repeat(32) } });
	...
	test("secret-safe lifecycle status never includes the raw authentication material", () => {
		const config = resolveBreadboardRunConfig({ ...configInput, cli: { engineMode: "remote", engineUrl: "https://engine.example/private" }, environment: { BREADBOARD_API_TOKEN: "v".repeat(32) } });
