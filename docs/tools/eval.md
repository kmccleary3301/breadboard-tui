...
- Spawn policy, discovered-agent availability, the `task.maxRecursionDepth` gate (default 2 in OMP and 1 in BreadBoard; negative values disable the cap), hard turn budget, subagent failure, strict schema failure, and isolation-apply failure are enforced as cell errors.
...
- Eval subagent spawning obeys `task.maxRecursionDepth` (default 2 in OMP and 1 in BreadBoard; negative values allow unlimited depth). Helper fan-out uses `task.maxConcurrency` (default 32 in OMP and 4 in BreadBoard; `0` is unbounded).
